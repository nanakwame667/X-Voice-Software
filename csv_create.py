...
directory = "D:/personnel/Semester Project/Voice-to-Age-master/"
wav_dir = os.listdir(directory)
total = 0
t = 0
...
with open("output-final-2.csv", 'w') as csvfile:
    ...
    for di in wav_dir:
        ...
        if not os.path.exists(temp + "/wav"):
            continue
        fav = os.listdir(temp + "/wav")
        total += int(len(fav))
        if not os.path.exists(temp+"/etc/README"):
            continue
